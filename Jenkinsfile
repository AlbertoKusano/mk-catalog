...
def helmDeploy(Map args) {
    ...
    if(args.context == 'arn:aws:eks:us-east-1:126155532006:cluster/lean-production' || args.context == 'arn:aws:eks:us-east-1:126155532006:cluster/lean-staging'){
        if (args.dry_run) {
            println "Running dry-run deployment"
             ...
             sh "helm upgrade -i ${args.name} ${args.chart_dir} -f ${args.chart_dir}/values.${args.stage}.yaml --kube-context ${args.context} --dry-run --debug"
        } else {
            ...
            sh "helm upgrade -i ${args.name} ${args.chart_dir} -f ${args.chart_dir}/values.${args.stage}.yaml --kube-context ${args.context}"
...
docker.withRegistry('126155532006.dkr.ecr.us-east-1.amazonaws.com') {
    stage('checkout'){
        checkout scm
    }

    stage('docker login'){
        sh "aws --region us-east-1 ecr get-login-password | docker login --password-stdin --username AWS \"126155532006.dkr.ecr.us-east-1.amazonaws.com\""
    }

    namespace = "default"

    def chart_dir = "./charts/lms-transfer"
    ...
    def project_name2 = 'lms-transfer'
    ...
    env.KUBE_CONTEXT = 'arn:aws:eks:us-east-1:126155532006:cluster/lean-production'
    env.STAGE = 'prod'
    ...
    {
        env.KUBE_CONTEXT = 'arn:aws:eks:us-east-1:126155532006:cluster/lean-staging'
        env.STAGE = 'qa'
    }
